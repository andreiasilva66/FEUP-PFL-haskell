...
compareStackElement :: Stack -> Bool
compareStackElement stack = case (top stack, top (pop stack)) of
  (IntElem x, IntElem y) -> x == y
  (BoolElem x, BoolElem y) -> x == y
  _                      -> error "compareStackElement: expected two elements of the same type on top of the stack"
...
run ( Equ :rest, stack, state) = run (rest, push (BoolElem (compareStackElement stack)) (pop (pop stack)), state)
...
run ( Branch c1 c2 :rest, stack, state) = run(if getBool stack == True then run(c1,stack,state) else run(c2,stack,state))
